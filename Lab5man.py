...
def main():
    ...
    x=2
    for i in range(1, 3):
        StitchedImage = StitchImages(BaseImage, image_paths[i+x])
        BaseImage = StitchedImage.copy()
        x = x + 2
    
    # Append the result to the stitched image array
    cv2.imwrite("Desktop\Stitched_Panorama1.jpg", BaseImage)
    image_stitches.append((BaseImage))
    
    # Reset BaseImage for stitching the next set of images
    BaseImage, _, _ = ProjectOntoCylinder(image_paths[1])
    x=2
    for i in range(2, 4):
        StitchedImage = StitchImages(BaseImage, image_paths[i+x])
        BaseImage = StitchedImage.copy()
        x+=2
    # Append the result to the stitched image array
    cv2.imwrite("Desktop\Stitched_Panorama2.jpg", BaseImage)
    image_stitches.append(BaseImage)

    BaseImage, _, _ = ProjectOntoCylinder(image_paths[2])
    x=2
    for i in range(3, 5):
        StitchedImage = StitchImages(BaseImage, image_paths[i+x])
        BaseImage = StitchedImage.copy()   
        x+=2
    cv2.imwrite("Desktop\Stitched_Panorama3.jpg", BaseImage)
    image_stitches.append(BaseImage) 
    # Stitching the first three images
    BaseImage1, _, _ = ProjectOntoCylinder(image_paths[1])
    # Stitch the first two images together
    StitchedImage1 = StitchImages(BaseImage1, image_stitches[0])
   
    FinalStitchedImage = StitchImages(StitchedImage1, image_stitches[2])
    
    # Write the final stitched image to a file
    cv2.imwrite("Desktop\Stitched_Panorama.jpg", FinalStitchedImage)
